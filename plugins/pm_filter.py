...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⬅️ Back", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"🌹 𝗣𝗮𝗴𝗲 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("Link Expired Please Search Again🙂.", show_alert=True)
    ...
    if k==False:
        ...
        if files:
            ...
            k = await query.message.edit('This Movie Is Not Released Or Is Not Added In My Database💌')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("🥺Friend Thats Not Fair 😓Please Join The Channel..🥺",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
